...
To update the database use this command from the Web project folder (replace `HeavyEquipmentRental` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../HeavyEquipmentRental.Infrastructure/HeavyEquipmentRental.Infrastructure.csproj -s HeavyEquipmentRental.Web.csproj
```
